...
dst=/home/lasser/agent_based_covid/agent_based_COVID_SEIRX/data/school/results_sensitivity_analysis/mask_efficiency


for stype in primary primary_dc lower_secondary lower_secondary_dc upper_secondary secondary
	do
	
	for m_idx in $(seq 0 47)
		do
		...
		echo "*********************"
		echo run_data_creation_mask_efficiency.py $stype $N_runs $m_idx $src $dst
		python run_data_creation_mask_efficiency.py $stype $N_runs $m_idx $src $dst &
		echo "*********************"
		sleep 1
