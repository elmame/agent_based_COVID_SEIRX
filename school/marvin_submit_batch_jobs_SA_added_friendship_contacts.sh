...
for stype in primary primary_dc lower_secondary lower_secondary_dc upper_secondary secondary
	...
	for m_idx in $(seq 0 59)
		...
		echo "*********************"
		echo run_data_creation_added_friendship_contacts.py $stype $N_runs $m_idx $src $dst
		python run_data_creation_added_friendship_contacts.py $stype $N_runs $m_idx $src $dst &
		echo "*********************"
		sleep 1
